...
//     const [loading,setLoading]=useState(false)
    ...
    //     console.log(search)
    //     let newText=search.split(" ");
    //     setSearch(newText.join(""))
    //     let url = `https://v1.nocodeapi.com/rautp/spotify/ejxNcuQXtrBlQYPR/search?q=${search}`
    ...
    // return (
        ...
        //     <input className="form-control " type="text" placeholder="Search" onChange={async (e) => { setSearch(e.target.value) }} id='search' aria-label="Search" />
        //    <div className='input-group-append mx-3'>
        //     <button className='btn btn-outline-success  my-2 my-sm-0' type='submit' onClick={searches} >search</button>
        //     </div>
        //     </div>
        //     </div>
        //     </div>
        //     <div className='row '>
        //         {loading===true && Object.keys(song).map((element) => {
        //             console.log(song)
        //             return <div className='col-md-4 my-3' key={song[element].external_urls.spotify}>
        //                 <Songslist songImages={song[element].images[0].url} artist={song[element].artists[0].name} release={song[element].release_date} name={song[element].name} songUrl={song[element].external_urls.spotify} />
        //             </div>
        //         })
